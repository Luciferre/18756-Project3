...
public class Computer implements IATMCellConsumer{
	...
	public void receiveCell(ATMCell cell, ATMNIC nic){
		if(cell.getIsOAM()){
			if(cell.getData().startsWith("call proceeding"))
				receivedCallProceeding(cell);
			else if(cell.getData().startsWith("wait"))
				receivedWait(cell);
			else if(cell.getData().startsWith("end ack"))
				receivedEndAck(cell);
			else if(cell.getData().startsWith("connect")){
				receivedConnect(cell);
				ATMCell cnackCell = new ATMCell(cell.getVC(),"connect ack",this.getTraceID());         
                System.out.println("The connection is setup on VC " + cell.getVC());
